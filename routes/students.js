...
router.get('/', async (req, res) => {
  try {
    ...
    // FIX: has_health_alert now checks ALL three health fields, not just allergies
    // FIX: balance uses GREATEST(..., 0) so overpayments don't turn balance negative
    const sql = `
      SELECT
        ...
        COALESCE(SUM(GREATEST(fp.amount_expected - fp.amount_paid, 0)), 0)        AS balance,
        CASE
          WHEN (hr.allergies         IS NOT NULL AND hr.allergies         <> 'None')
            OR (hr.chronic_conditions IS NOT NULL AND hr.chronic_conditions <> 'None')
            OR (hr.current_medication IS NOT NULL AND hr.current_medication <> 'None')
          THEN TRUE ELSE FALSE
        END AS has_health_alert
      FROM students s
      LEFT JOIN classes        c  ON s.class_id  = c.id
      LEFT JOIN fee_payments   fp ON s.id        = fp.student_id
      LEFT JOIN health_records hr ON s.id        = hr.student_id
      ${whereClause}
      GROUP BY s.id, c.name, c.id, hr.allergies, hr.chronic_conditions, hr.current_medication
      ORDER BY s.name
...
// ─────────────────────────────────────────────
// GET single student (with fees, academics, health, attendance)
// ─────────────────────────────────────────────
  ...
  try {
    ...
    if (!adm_no || !name || !parent_phone) {
      ...
      return res.status(400).json({
        success: false,
        message: 'Admission number, name and parent phone are required.',
      });
    }

    // Duplicate admission number check
    const existing = await client.query(
      'SELECT id FROM students WHERE adm_no = $1',
      [adm_no]
    );
    ...
    const studentResult = await client.query(
      `INSERT INTO students
         (adm_no, name, class_id, gender, date_of_birth,
          parent_name, parent_phone, parent_email, address, county, year_joined)
    ...
    // ── CRITICAL FIX: define student HERE, before the fee block ────────────
    const student = studentResult.rows[0];

    // ── AUTO-GENERATE FEE RECORD FOR CURRENT TERM ──────────────────────────
    // Wrapped in try/catch so a fee config gap never blocks student registration
    try {
      // Prefer the is_current flag; fall back to today's date range
      ...
      if (currentTerm && class_id) {
        // Only insert if no fee record already exists for this student+term
        ...
        if (existingFee.rows.length === 0) {
          // Find the fee structure for this class + term
          ...
          if (feeStructure.rows.length > 0) {
            const termFee = parseFloat(feeStructure.rows[0].total_amount || 0);

            await client.query(
              `INSERT INTO fee_payments
                 (student_id, term, academic_year, amount_expected, amount_paid, payment_method, received_by)
            ...
            console.log(`Auto-generated fee record for ${student.name}: ${currentTerm.term} — KES ${termFee}`);
          } else {
            console.log(`No fee structure found for class ${class_id}, ${currentTerm.term}`);
          }
        }
      }
    } catch (feeErr) {
      // Log but NEVER block the student registration
    ...
    try {
      await client.query(
        `INSERT INTO health_records
           (student_id, blood_group, allergies, chronic_conditions, current_medication, emergency_contact_phone)
         VALUES ($1,$2,$3,$4,$5,$6)`,
        [
          student.id,
          blood_group,
...
// ─────────────────────────────────────────────
// PUT update student (basic info + health)
// ─────────────────────────────────────────────
router.put('/:id', authorize('admin', 'principal'), async (req, res) => {
  try {
    const {
      name, class_id, gender, date_of_birth,
      parent_name, parent_phone, parent_email, address, county,
      blood_group, allergies, chronic_conditions, current_medication, emergency_contact_phone,
    } = req.body;
    ...
    const result = await query(
      ...
      [name, class_id, gender, date_of_birth || null,
       parent_name, parent_phone, parent_email,
       address, county, req.params.id]
    );
    ...
    await query(
      ...
      [blood_group, allergies, chronic_conditions, current_medication,
       emergency_contact_phone, req.params.id]
    );
...
// ─────────────────────────────────────────────
// DELETE (soft-deactivate) student
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// GET students by class (for marks / attendance)
// ─────────────────────────────────────────────
...
router.get('/export/excel', async (req, res) => {
  try {
    ...
    let sql = `
      SELECT s.adm_no, s.name, s.gender, s.date_of_birth, c.name AS class_name,
    ...
    students.forEach((s, idx) => {
      const row = sheet.addRow([
        ...
        s.class_name || '', s.parent_name || '', s.parent_phone || '',
        s.parent_email || '', s.address || '', s.county || '', s.year_joined || '',
