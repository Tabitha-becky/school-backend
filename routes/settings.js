...
router.get('/current-term', async (req, res) => {
  try {
    ...
    // Fallback: get the most recently set current term
    ...
    // Last resort: derive from current date
    ...
    res.json({ success: true, data: { academic_year: String(year), term, term_start_date: null, term_end_date: null } });
  } catch (err) {
...
router.get('/terms', async (req, res) => {
  try {
    const result = await query('SELECT * FROM school_settings ORDER BY term_start_date DESC');
...
router.post('/terms', authorize('admin', 'principal'), async (req, res) => {
  try {
    const { academic_year, term, term_start_date, term_end_date, is_current } = req.body;
    ...
    const result = await query(
      `INSERT INTO school_settings (academic_year, term, term_start_date, term_end_date, is_current)
       VALUES ($1,$2,$3,$4,$5)
       ON CONFLICT DO NOTHING RETURNING *`,
      [academic_year, term, term_start_date, term_end_date, is_current || FALSE]
    );
